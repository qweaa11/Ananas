...
public class NSYLibraryController {
	...
	//==== 양반양 암호화 알고리즘 AES256 의존객체 주입하기 (DI : Dependency Injection) ====
	@Autowired
	private AES256 aes;
	...
	public String resisterLibraryEnd(NSYLibraryVO libraryvo, MultipartHttpServletRequest req) {
		...
		if(!imgFile.isEmpty()) { // imgFile(첨부파일) 있는경우 
			...
			try {
				...
				//getBytes()는 첨부된 파일을 바이틑 단위로 파일을 다 읽어오는 것이다.
